...
> **상태 (2026-09-03)**: AI 측 계약은 KAN-9 §5·§7에 정렬 완료(→ `KAN-17`). 공개 API JSON은 §3을 도윤에게 전달하고 노션 §4와 차이를 대조하면 KAN-4가 닫힌다.
...
| 프론트 담당자가 추가 설명 없이 입력·결과 화면을 연결할 수 있다 | 🟡 OpenAPI + 예시 13개 완성(`docs/openapi/`, 테스트 47). 도윤 확인 2건(§3 결정 1·2) 후 ✅ |
| AI 응답은 KAN-12의 가드레일과 입력·출력 규격을 따른다 | ✅ 테스트 135/135 (공개 응답의 `explanation` == KAN-12 픽스처를 `test_public_api`가 확인) |
