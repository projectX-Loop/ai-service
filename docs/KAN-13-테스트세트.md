...
| A10 | **선택된 주기 반영** | 사용자가 고른 주기를 설명에서 누락 | `highlighted_frequency` == 입력 `selected_frequency`, 해당 주기가 `summary`/`goal_gap`에 등장. PRD 수용기준 4 |
...
| 케이스 | 픽스처 | 상태 |
|---|---|---|
| 1 간극 작음 | `case1_small_gap.json` + `case1_response_good.json` | 작성 완료 |
| 2~5 | — | **미작성.** KAN-11 결과 JSON 계약 확정 후 작성 |
| 6 입력 오류 | — | KAN-4 구현 후 API 레벨에서 검증 |
